...
public class ValidationHelper {
	...
	public static ChangesStatus isValidAccountId(String account_id) 
	{
		try 
		{
			...
			e.printStackTrace();
			return new ChangesStatus(false,"Account ID doesn't exit. Transaction Cancled.");
	...
	public static ChangesStatus isValidSecurityPin(String pinIn) 
	{
		...
		try 
		{
			if (pinIn.length() > 6 || pinIn.length() < 6) {
				return new ChangesStatus(false,"Security Pin is a 6 digit number.");
			}
			Integer.parseInt(pinIn);
			return new ChangesStatus(true, "");
		} catch (NumberFormatException e) {
			return new ChangesStatus(false,"Security Pin must be only numbers.");
		}
	}
